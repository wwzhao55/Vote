...
+ function($) {
  ...
  var Picker = function (params) {
      ...
      var defaults = {
          ...
          <a href="javascript:;" class="picker-button cancel-picker">取消</a>\
      ...
      // Open
      function onPickerClose(type) {
          p.opened = false;
          if (p.input && p.input.length > 0) p.input.parents('.page-content').css({'padding-bottom': ''});
          if (p.params.onClose) {
            if(type != 'cancel'){
              p.params.onClose(p,'close');
            }else{
              p.params.onClose(p,'cancel');
            }  
          }
